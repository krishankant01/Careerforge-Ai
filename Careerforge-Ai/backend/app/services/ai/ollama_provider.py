...
class OllamaProvider(AIProvider):
    ...
    async def generate_json(
        ...
        import re
        text = await self.generate_text(system_prompt, user_prompt, max_tokens=max_tokens, force_json=True)
        text = text.strip()
    ...
    async def generate_text(
        self, system_prompt: str, user_prompt: str, *, max_tokens: int = 1200, force_json: bool = False
    ) -> str:
        try:
            payload = {
                "model": self.model,
            ...
            async with httpx.AsyncClient(timeout=120) as client:
                ...
                if not res_text and data.get("thinking"):
                    res_text = str(data.get("thinking")).strip()
                return res_text
